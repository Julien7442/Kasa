...
export default function FicheLogement() {
  ...
  useEffect(() => {
    const getData = async () => {
      const res = await axios.get('/logements.json'); //Utilisation d'axios pour mise en place d'API
      const picked = res.data.find(({ id }) => id === params.id);
      res.data.map(() => setPickedAppart(picked));
      if (picked === undefined) {
        navigate('/404', { state: { message: "Can't get data" } }); //renvoi vers la page 404 si lien invalide
      }
    };
    getData();
    // eslint-disable-next-line
  }, []); // L'array de useEffect est vide et lancer une seule fois
