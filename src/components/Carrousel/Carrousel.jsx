...
export default function Carrousel({ slides }) {
  const [current, setCurrent] = useState(0); // Index du premier slide a 0
  const length = slides.length; // Longueur du tableau slide

  const nextSlide = () => {
    setCurrent(current === length - 1 ? 0 : current + 1); // Arriver au dernier slide, on repart au premier
  };
  const prevSlide = () => {
    setCurrent(current === 0 ? length - 1 : current - 1); // Arriver au dernier slide, on repart au premier
  };
